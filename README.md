...
하루 24시간을 **피자 형태의 원형 파이 차트**로 시각화하고 관리하는 로컬 우선 웹 앱입니다.
...
- **로컬 우선 저장** — 기본 일정 편집은 로그인 없이 사용하며 localStorage에 자동 저장. 오프라인 편집을 지원하고, 계정·Pro 클라우드 동기화·결제·푸시·뉴스 등 네트워크 기능은 별도로 제공.
...
현재 진입점, 상태 흐름, Cloudflare Worker API 및 저장 구조는 [아키텍처 안내](docs/ARCHITECTURE.md)를 참고하세요. 아래 트리는 핵심 편집기 파일을 요약합니다.
...
- **로컬 우선 SPA + 선택적 클라우드 기능** — 편집 데이터는 localStorage에 저장하고, Google 로그인과 Pro 동기화 등은 Cloudflare Worker/D1을 사용합니다. 주요 저장 데이터는 버전 봉투(`{ version, ... }`)를 사용하며, 동기화에는 3-way 병합과 선택적 종단간 암호화를 적용합니다.
