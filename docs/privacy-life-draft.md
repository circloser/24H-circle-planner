...
> **게시 완료(2026-09-20).** 이 초안을 다듬어 `/privacy`(public/privacy.html)의
> "라이프(인생 타임라인) / Life timeline" 절로 넣었고, 시행일도 갱신했습니다.
> 아래 내용은 작성 근거로 남겨 둡니다.
...
- **부모님 정보(제3자 정보)**: 어머니·아버지의 이름, 생년월일(선택), 메모(선택), 사진(Pro)
...
- **사진**은 Pro에서만 넣을 수 있으며, 사진을 추가한 기기의 브라우저(IndexedDB)에만 저장됩니다. Pro 동기화를 켜도 사진은 동기화되지 않고 서버로 보내지지 않습니다.
- **이미지(PNG) 내보내기**와 **JSON 백업**은 이용자의 기기 안에서 만들어져 이용자가 고른 위치에 저장되며, 서버로 보내지지 않습니다. 이후 이 파일을 누구와 공유할지는 이용자가 정합니다.

**부모님 정보에 관하여**

- 부모님의 이름·생년월일 등은 이용자가 아닌 제3자의 개인정보입니다. 이용자는 부모님의 동의를 받아 기록해야 하며, 특히 Pro 동기화를 켜면 해당 정보가 서버에도 저장된다는 점을 알려 주시기 바랍니다.
- 회사는 부모님 정보를 이용자의 기록을 보관·동기화하는 목적 외에는 이용하지 않으며, 마케팅이나 분석에 쓰지 않습니다.
...
- **Parents (third-party information)**: your mother's and father's names, birthdays (optional), notes (optional) and photos (Pro)
...
- **Photos** can be added with Pro only and stay in the browser (IndexedDB) of the device that added them. Even with Pro sync on, they are not synced and never sent to our server.
...
**About your parents' details**

- Your parents' names and birthdays are personal information about other people. Please record them with their consent, and let them know that turning on Pro sync also stores them on our server.
- We use these details only to keep and sync your record — never for marketing or analysis.
...
- We never pass the ending note on to anyone.
