...
/** Darken a hex colour for the folded-corner flap. */
...
export function MemoNote({ memo }: { memo: Memo }) {
  ...
  return (
    ...
    >
      {/* Hover toolbar — colours + font + delete, above the note. */}
        ...
        <div className="flex items-center gap-1">
          {MEMO_COLORS.map((c) => (
            <button
              ...
              style={{ backgroundColor: c, outline: memo.color === c ? '2px solid rgba(0,0,0,0.45)' : 'none', outlineOffset: '1px' }}
            />
          ))}
        </div>
        <select
      ...
      <div className="memo-fold" style={{ backgroundColor: darken(memo.color) }} />
    </div>
  );
}
